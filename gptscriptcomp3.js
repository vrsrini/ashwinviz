...
function startVisualization() {
	console.log("Entered startVisualization function");
    /*d3.csv("bowler_dataNew.csv").then(data => {
...
/*       // Check if data needs to be reloaded (e.g., loadedData is empty)
        if(loadedData.length === 0) {
            startVisualization(); // Reload or initialize data
...
if (maxWickets === 100) {
    ...
    milestones = [550]; // Add 200 as a second milestone to track
}

let currentMilestoneIndex = 0; // To track which milestone we're currently checking

/*// Check milestones and update race status
matchData.forEach(d => {
    if (milestoneCount < 5 && d.wicketsTaken >= milestoneThreshold && !resumePoints.includes(d.Name)) {
        racePaused = true;			
			console.log("Checking for d.Name");
			console.log(d.Name);
			let matchKey = `${d.matchesPlayed} Tests`;
			if (!milestonesByMatch[matchKey]) {
				milestonesByMatch[matchKey] = [];
				milestoneCount++;
			}
			// Now pushing an object with both name and team
			milestonesByMatch[matchKey].push({ name: d.Name, team: d.Team });
			console.log(milestonesByMatch[matchKey]);
...
// Display milestone messages dynamically based on milestoneThreshold
document.getElementById('milestone-message').innerText = '';
Object.entries(milestonesByMatch).forEach(([matches, bowlers]) => {
    let bowlerDetails = bowlers.map(bowler => `${bowler.name} (${bowler.team})`).join(", ");
    document.getElementById('milestone-message').innerText += `Bowler(s) who reached ${milestoneThreshold} wickets in ${matches} - ${bowlerDetails}\n`;
});*/
...
matchData.forEach(d => {
    ...
    if (milestoneCount < 5 && d.wicketsTaken >= currentMilestone && !resumePoints.includes(d.Name)) {
        racePaused = true;
	...
	/*if (winner) {
		raceFinished = true;
		// Append the winner message to the milestone message box in bold font
		...
		document.getElementById('milestone-message').innerHTML += winnerMessage;
		resetForNewRace();
		//displaySummaryBox(winner.matchesPlayed); // Assuming you want to display the summary box here as well
	}*/
		if (winner) {
		raceFinished = true;
		let winnerMessage = `<strong>Race finished! Bowler ${winner.Name} wins by reaching ${maxWickets} wickets in ${winner.matchesPlayed} Test matches.</strong>`;
	...
	/*
    let winner = matchData.find(d => d.wicketsTaken >= 200);
    if (winner) {
        raceFinished = true;
        alert(`Race finished! Bowler ${winner.Name} wins by reaching 200 wickets.`);
		displaySummaryBox(); 
    }*/
}
...
/*function runRace() {
    if (!raceFinished && !racePaused) {
        ...
        if (matchData.length > 0) {
            ...
            currentMatch++;
            // Defer the next call to runRace to allow UI updates and avoid freezing
...
function resetForNewRace() {
    ...
    document.getElementById('maxWickets').value = "200"; // or any default value you prefer
...
// Tooltip functions for mouseover and mouseout events
/*
