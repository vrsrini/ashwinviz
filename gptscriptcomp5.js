...
function startVisualization() {
	return d3.csv("bowler_dataNew.csv").then(data => { // Return the promise here
        ...
        window.addEventListener('scroll', function() {
            const raceDescriptionWrapper = document.querySelector('.race-description-wrapper');
            const wrapperTop = raceDescriptionWrapper.getBoundingClientRect().top;

            const controlsContainer = document.querySelector('.controls-container');
            controlsContainer.style.top = (wrapperTop + raceDescriptionWrapper.offsetHeight + 20) + 'px'; 

            const milestoneMessages = document.querySelector('.milestone-messages'); 
            milestoneMessages.style.top = (wrapperTop + raceDescriptionWrapper.offsetHeight + 20) + 'px';  
        });
		
		chartContainer = document.getElementById('chart');
		chartOffset = chartContainer.offsetTop;
    ...
    bars.enter()
		...
		.style("fill", function(d) {
		  if (d.Name === "R Ashwin") {
			  ...
			  return d.Type === "Spin" ? "#4791FF" : "#BC4602"; // Existing logic
			}		
		  })
...
function calculateViewportHeight(wickets) {
    ...
    console.log("yPixelValue:", yPixelValue); 
    // Calculate based on fixed chart height
    const maxScroll = height - getAccurateViewportHeight() + pixelOffset; 
    return Math.min(yPixelValue + pixelOffset, maxScroll); 
}

function getAccurateViewportHeight() {
...
/*// Modify your calculateViewportHeight function:
function calculateViewportHeight(wickets) {
    const yPixelValue = y(wickets); 
    const pixelOffset = 50; 
    const maxScroll = height - getAccurateViewportHeight() + pixelOffset; 
...
matchData.forEach(d => {
    ...
    if ((milestoneCount < 5 || maxWickets === 800) && d.wicketsTaken >= currentMilestone && !resumePoints.includes(d.Name)) {
		...
		// Scrolling during pause
			...
			console.log("Current Milestone (wickets):", currentMilestone);
            const viewportHeight = calculateViewportHeight(currentMilestone);
			// Inside the 'if (racePaused)' block, after calculating viewportHeight:
			window.scrollTo({
				top: viewportHeight + chartOffset, 
				behavior: 'smooth'
			});
	}
